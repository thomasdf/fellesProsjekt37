...
/**
 * A Group-class acts in very much the same way as an {@link Account}-class,
 * although the Group-class will ""belong"" to more than one user, which will
 * lead to more than one user being able to see, edit and in other ways modify
 * the {@link Calendar} it's linked to.
 * 
 * @author gruppe37
 * @version %I%, %G%
 */
public class Group {
	...
	// Property-attributes
	private ObservableList<String> membersList = FXCollections
			.observableList(new ArrayList<String>());
	private ObservableList<Integer> in_groupsList = FXCollections
			.observableList(new ArrayList<Integer>());
	private ObservableList<Integer> subgroupsList = FXCollections
			.observableList(new ArrayList<Integer>());
	private ObservableList<Integer> calendarsList = FXCollections
			.observableList(new ArrayList<Integer>());
	...
	/*
	 * Oppsett for checkbox i tableview. Checked variablen tilsier om denne
	 * brukeren er huket av i invite viewet eller ikke. Pass på å alltid sette
	 * den til "false" for ALLE brukere når du entrer invite viewet. Hvis ikke
	 * vil systemet tro at brukeren alltid er huket av, og dermed bli invitert
	 * til alt.
	 */
